...
class SQ_Classes_Helpers_Sanitize {
	...
	/**
	 * Convert a country name (or a nested Country/PostalAddress value) into its
	 * ISO 3166-1 alpha-2 code (e.g. "Romania" => "RO"). Returns an already-valid
	 * 2-letter code untouched, and the original value when it can't be mapped.
	 * Self-contained on purpose so it works without the other plugin present.
	 *
	 * @param mixed $value
	 *
	 * @return string
	...
	public static function getCountryCode( $value ) {
		...
		$map = array(
			...
			'america' => 'US', 'american samoa' => 'AS', 'andorra' => 'AD', 'angola' => 'AO',
			'anguilla' => 'AI', 'antarctica' => 'AQ', 'antigua' => 'AG', 'antigua and barbuda' => 'AG',
			'argentina' => 'AR', 'armenia' => 'AM', 'aruba' => 'AW', 'australia' => 'AU',
			'austria' => 'AT', 'azerbaijan' => 'AZ', 'bahamas' => 'BS', 'bahrain' => 'BH',
			'bangladesh' => 'BD', 'barbados' => 'BB', 'belarus' => 'BY', 'belgium' => 'BE',
			'belize' => 'BZ', 'benin' => 'BJ', 'bermuda' => 'BM', 'bhutan' => 'BT',
			'bolivia' => 'BO', 'bonaire' => 'BQ', 'bosnia' => 'BA', 'bosnia and herzegovina' => 'BA',
			'botswana' => 'BW', 'bouvet island' => 'BV', 'brazil' => 'BR', 'british indian ocean territory' => 'IO',
			'british virgin islands' => 'VG', 'brunei' => 'BN', 'brunei darussalam' => 'BN', 'bulgaria' => 'BG',
			...
			'czechia' => 'CZ', 'côte d\'ivoire' => 'CI', 'democratic republic of the congo' => 'CD', 'denmark' => 'DK',
			'djibouti' => 'DJ', 'dominica' => 'DM', 'dominican republic' => 'DO', 'dr congo' => 'CD',
			'east timor' => 'TL', 'ecuador' => 'EC', 'egypt' => 'EG', 'el salvador' => 'SV',
			'england' => 'GB', 'equatorial guinea' => 'GQ', 'eritrea' => 'ER', 'estonia' => 'EE',
			'eswatini' => 'SZ', 'ethiopia' => 'ET', 'falkland islands' => 'FK', 'faroe islands' => 'FO',
			...
			'jordan' => 'JO', 'kazakhstan' => 'KZ', 'keeling islands' => 'CC', 'kenya' => 'KE',
			'kiribati' => 'KI', 'korea democratic people\'s republic' => 'KP', 'korea republic' => 'KR', 'kuwait' => 'KW',
			'kyrgyzstan' => 'KG', 'lao' => 'LA', 'laos' => 'LA', 'latvia' => 'LV',
			...
			'pakistan' => 'PK', 'palau' => 'PW', 'palestine' => 'PS', 'palestinian territory' => 'PS',
			...
			'rwanda' => 'RW', 'réunion' => 'RE', 'saint barthelemy' => 'BL', 'saint barthélemy' => 'BL',
			...
			'somalia' => 'SO', 'south africa' => 'ZA', 'south georgia' => 'GS', 'south korea' => 'KR',
			'south sudan' => 'SS', 'spain' => 'ES', 'sri lanka' => 'LK', 'sudan' => 'SD',
			'suriname' => 'SR', 'svalbard and jan mayen' => 'SJ', 'swaziland' => 'SZ', 'sweden' => 'SE',
			'switzerland' => 'CH', 'syria' => 'SY', 'syrian arab republic' => 'SY', 'são tomé and príncipe' => 'ST',
			'taiwan' => 'TW', 'tajikistan' => 'TJ', 'tanzania' => 'TZ', 'thailand' => 'TH',
			'the netherlands' => 'NL', 'timor-leste' => 'TL', 'togo' => 'TG', 'tokelau' => 'TK',
			'tonga' => 'TO', 'trinidad and tobago' => 'TT', 'tunisia' => 'TN', 'turkey' => 'TR',
			'turkiye' => 'TR', 'turkmenistan' => 'TM', 'turks and caicos islands' => 'TC', 'tuvalu' => 'TV',
			'türkiye' => 'TR', 'u.s.' => 'US', 'u.s. virgin islands' => 'VI', 'u.s.a.' => 'US',
			...
			'united states of america' => 'US', 'uruguay' => 'UY', 'usa' => 'US', 'uzbekistan' => 'UZ',
			'vanuatu' => 'VU', 'vatican' => 'VA', 'vatican city' => 'VA', 'venezuela' => 'VE',
			'viet nam' => 'VN', 'vietnam' => 'VN', 'virgin islands british' => 'VG', 'virgin islands u.s.' => 'VI',
			'wales' => 'GB', 'wallis and futuna' => 'WF', 'western sahara' => 'EH', 'yemen' => 'YE',
		...
		//otherwise, accept an already-valid 2-letter ISO code as-is
	...
	/**
	 * Filter HTML that arrives from the Squirrly Cloud before it is printed in wp-admin.
	 *
	 * The cloud is trusted, but it is still a remote source: a compromised response or a
	 * MITM would otherwise be able to run script in an administrator's browser. This keeps
	 * the display markup those notices use (layout, links, images, inline styles, SVG icons)
	 * and drops everything that carries behaviour: script/style/iframe/form/input elements,
	 * on* event handlers, and every data-* attribute (the admin JS eval()s data-javascript
	 * and fires privileged ajax actions from data-action/data-name/data-value).
	 *
	 * @param string $html
	 *
	 * @return string
	 */
	public static function cloudHtml( $html ) {

		if ( ! is_string( $html ) || $html === '' ) {
			return '';
		}

		//drop script/style blocks together with their contents - wp_kses() only removes the
		//tags, which would leave the raw JS or CSS behind as visible text in the notice
		$html = preg_replace( '#<\s*(script|style)\b[^>]*>.*?<\s*/\s*\1\s*>#is', '', $html );
		$html = preg_replace( '#<\s*(script|style)\b[^>]*/?>#i', '', $html );

		//defence in depth: strip the attributes that drive behaviour before wp_kses runs, so
		//even if the allowlist below is ever loosened they can't come back. The plugin's admin
		//JS eval()s data-javascript and fires privileged ajax actions from
		//data-action/data-name/data-value (see sq_ajaxCallListen), so remote HTML must never
		//carry them, and on* event handlers are always unsafe.
		$html = preg_replace( '#\s(?:data-javascript|data-action|data-name|data-value|data-input|data-confirm|data-redirect|on[a-z]+)\s*=\s*("[^"]*"|\'[^\']*\'|[^\s>]+)#i', '', $html );

		$allowed = wp_kses_allowed_html( 'post' );

		//display-only attributes. Deliberately NO data-* wildcard and no behaviour hooks - a
		//tampered cloud response must not be able to run script or trigger actions in wp-admin.
		$extra = array(
			'class'          => true,
			'id'             => true,
			'style'          => true,
		...
		//nav is included because Squirrly's own admin notices are built as <nav> wrappers
		foreach ( array( 'nav', 'div', 'span', 'p', 'a', 'ul', 'ol', 'li', 'img', 'strong', 'em', 'b', 'i', 'small', 'br', 'h1', 'h2', 'h3', 'h4', 'h5', 'h6', 'table', 'thead', 'tbody', 'tr', 'td', 'th', 'button' ) as $tag ) {
			if ( ! isset( $allowed[ $tag ] ) ) {
				$allowed[ $tag ] = array();
			}

			$allowed[ $tag ] = array_merge( (array) $allowed[ $tag ], $extra );
		}

		//buttons are not in the default post allowlist
		...
		//inline SVG icons - safe once wp_kses has removed the on* handlers, and notices
		//lose their icon without them. Geometry/presentation attributes only.
		$svg_attr = array(
			'xmlns'        => true,
			'viewbox'      => true,
			'width'        => true,
		...
		foreach ( array( 'svg', 'g', 'path', 'circle', 'ellipse', 'rect', 'line', 'polyline', 'polygon', 'title' ) as $tag ) {
			$allowed[ $tag ] = $svg_attr;
		}

		//NOTE: form/input and all data-*/on* attributes are deliberately NOT allowed - a
		//tampered cloud response must not be able to render a credential form, embed
		//eval-able attributes, or trigger privileged ajax actions inside wp-admin.

		//Keep WordPress's CSS value filtering - it blocks the executable pieces (url(),
		//expression(), behavior, @import) - but let the notice keep its designed styling.
		//Vanilla safecss otherwise drops legitimate declarations such as a gradient that
		//carries !important, leaving an offer banner unstyled.
		$css_guard = function ( $allow, $css ) {
			if ( preg_match( '/url\s*\(|expression\s*\(|javascript\s*:|vbscript\s*:|@import|behaviou?r\s*:|-moz-binding/i', (string) $css ) ) {
				return false;
			}
		...
		add_filter( 'safecss_filter_attr_allow_css', $css_guard, 10, 2 );
		$safe = wp_kses( $html, $allowed );
		remove_filter( 'safecss_filter_attr_allow_css', $css_guard, 10 );
