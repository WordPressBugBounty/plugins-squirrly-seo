...
class SQ_Classes_RemoteController
{
    ...
    /**
     * Ask the Cloud for a one-time sign-in link for this account.
     *
     * Called only when the user actually clicks a Cloud link (see sq_account_cloudlogin in
     * SQ_Controllers_Account), never while rendering one - the returned link is single-use, so
     * minting it per rendered link would leave every link but the first one dead.
     *
     * Returns a WP_Error for DevKit (Business/Agency) accounts, which the Cloud excludes from
     * automatic sign-in, and for any failure. Callers must fall back to a plain Cloud URL.
     *
     * @param  array $args ('redirect' => relative Cloud path such as 'account')
     * @return string|WP_Error absolute Cloud URL
     */
    public static function getDashboardLink($args = array())
    {
        self::$apimethod = 'get'; //call method

        $json = json_decode(self::apiCall('api/user/dashboardlink', $args));

        //Every failure below ends in the same silent fallback to a plain Cloud URL, so record the
        //reason: without it "it just goes to the login page" is indistinguishable between a DevKit
        //account, an undeployed route, and a Cloud outage. Readable afterwards from the
        //sq_last_api_debug transient (and error_log when WP_DEBUG is on).
        if (isset($json->error) && $json->error <> '') {
            return (new WP_Error('api_error', $json->error, self::debugFailure('api/user/dashboardlink', $json->error)));
        } elseif (!isset($json->data->url) || $json->data->url == '') {
            return (new WP_Error('api_error', 'no_data', self::debugFailure('api/user/dashboardlink', 'no_data')));
        }

        $url = (string) $json->data->url;

        //Only ever redirect to the Cloud itself: a compromised or spoofed response must not be able
        //to bounce a logged-in admin to an arbitrary host.
        if (strpos($url, _SQ_DASH_URL_) !== 0) {
            return (new WP_Error('api_error', 'invalid_url', self::debugFailure('api/user/dashboardlink', 'invalid_url: ' . $url)));
        }

        $query = array();
        if (isset($json->data->redirect) && $json->data->redirect <> '') {
            $query['redirect'] = (string) $json->data->redirect;
        }
        if (!empty($json->data->blog_id)) {
            $query['blog'] = (int) $json->data->blog_id;
        }
