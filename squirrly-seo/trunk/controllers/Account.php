	...
	public function action() {
		switch ( SQ_Classes_Helpers_Tools::getValue( 'action' ) ) {
			case 'sq_account_disconnect':
				...
				break;
			case 'sq_account_cloudlogin':

				if ( ! SQ_Classes_Helpers_Tools::userCan( 'sq_manage_settings' ) ) {
					return;
				}

				//Where on the Cloud the user wanted to go. Kept to a bare relative path - the Cloud
				//feeds it to admin_url(), and it is re-sanitized there too.
				$path = preg_replace( '#[^A-Za-z0-9/_-]#', '', (string) SQ_Classes_Helpers_Tools::getValue( 'sq_path', 'dashboard' ) );
				$path = ltrim( $path, '/' );

				//Same buffering as sq_account_reconnect: we are on admin_menu, before the redirect,
				//so an SQ_DEBUG dump or a checkin notice emitted during the call would send headers
				//early and make the redirect silently fail.
				ob_start();
				$link = SQ_Classes_RemoteController::getDashboardLink( array( 'redirect' => $path ) );
				ob_end_clean();

				//No one-time link: a DevKit (Business/Agency) account, which the Cloud excludes from
				//automatic sign-in, or the Cloud was unreachable. Send the user to the Cloud page
				//anyway and let it ask them to log in - never fall back to putting a token in the URL.
				if ( is_wp_error( $link ) || ! $link ) {
					$link = _SQ_DASH_URL_ . 'user/' . $path;
				}

				//External host by design, so wp_redirect(); getDashboardLink() has already checked
				//that the Cloud returned a URL on _SQ_DASH_URL_.
				wp_redirect( $link );
				exit();
