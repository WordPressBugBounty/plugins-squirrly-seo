...
class SQ_Controllers_FocusPages extends SQ_Classes_FrontController {
	...
	public function action() {
		...
		if ( ! SQ_Classes_Helpers_Tools::userCan( 'sq_manage_focuspages' ) ) {
			...
			//stop here - without the return the actions below used to run anyway on non-ajax requests
			return;
		}
		...
		switch ( SQ_Classes_Helpers_Tools::getValue( 'action' ) ) {
			...
			case 'sq_focuspages_linkopportunities':
				...
				//this is part of Inner Links, gated like every other case here by
				//sq_manage_focuspages; it used to require sq_manage_settings, which no
				//Editor/Squirrly SEO Editor holds, so the search always failed for them
				if ( ! SQ_Classes_Helpers_Tools::userCan( 'sq_manage_focuspages' ) ) {
					wp_send_json_error( esc_html__( "You do not have permission to perform this action", 'squirrly-seo' ) );
				}
