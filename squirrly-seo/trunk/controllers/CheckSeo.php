			...
			foreach ( $report as $function => &$row ) {
				...
				if ( is_array( $row ) && isset( $tasks[ $function ] ) ) {
					...
					if ( ! $tasks[ $function ]['positive'] && ( $row['status'] == '' || ! empty( $tasks_for_today[ gmdate( 'Y-m-d' ) ] ) ) ) {
						...
						//replace links
						$row['warning']  = preg_replace_callback( '/\[link\]([^\[]*)\[\/link\]/i', function ( $m ) {
							return '<a href="' . esc_url( $m[1] ) . '" target="_blank">' . esc_html( $m[1] ) . '</a>';
						}, $row['warning'] );
						$row['message']  = preg_replace_callback( '/\[link\]([^\[]*)\[\/link\]/i', function ( $m ) {
							return '<a href="' . esc_url( $m[1] ) . '" target="_blank">' . esc_html( $m[1] ) . '</a>';
						}, $row['message'] );
						$row['solution'] = preg_replace_callback( '/\[link\]([^\[]*)\[\/link\]/i', function ( $m ) {
							return '<a href="' . esc_url( $m[1] ) . '" target="_blank">' . esc_html( $m[1] ) . '</a>';
						}, $row['solution'] );
