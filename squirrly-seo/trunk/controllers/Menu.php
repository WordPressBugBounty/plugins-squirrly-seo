...
class SQ_Controllers_Menu extends SQ_Classes_FrontController {
	...
	public function hookHead() {
		...
		if ( isset( $sq_setting_page ) && $sq_setting_page ) {
			...
			wp_enqueue_script( 'crawlbrain-widget', 'https://cdn.crawlbrain.com/v2/widget.min.js', array(), null );
	 ...
	 */
	/**
	 * Build the link behind every "go to my Squirrly Cloud" element.
	 *
	 * It points at this site, not at the Cloud: sq_account_cloudlogin (SQ_Controllers_Account)
	 * fetches a one-time sign-in link when the link is actually clicked, then redirects. The link
	 * used to carry `?token=<sq_api>` - the account-wide token, valid forever, for every website on
	 * the account - straight into the page HTML, where it leaked through browser history, the
	 * Referer sent to the Cloud, and any proxy or CDN log in between. Nothing secret is rendered now.
	 *
	 * @param string $url  the Cloud root, used as-is when the user can't reach the account
	 * @param string $path relative Cloud destination ('dashboard', 'account', 'plans', ...)
	 *
	 * @return string
	 */
		...
		if ( function_exists( 'wp_get_current_user' ) && SQ_Classes_Helpers_Tools::getOption( 'sq_api' ) ) {
			if ( SQ_Classes_Helpers_Tools::getMenuVisible( 'show_account_info' ) && SQ_Classes_Helpers_Tools::userCan( 'sq_manage_settings' ) ) {
				return esc_url( wp_nonce_url(
					admin_url( 'admin.php?page=sq_dashboard&action=sq_account_cloudlogin&sq_path=' . rawurlencode( $path ) ),
					'sq_account_cloudlogin',
					'sq_nonce'
				) );
			}
		}
