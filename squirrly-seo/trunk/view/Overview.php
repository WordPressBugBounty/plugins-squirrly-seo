...
<div id="sq_wrap" class="sq_overview">
		...
		<?php if ( SQ_Classes_Helpers_Tools::getOption( 'sq_onboarding' ) && SQ_Classes_Helpers_Tools::getOption( 'sq_notification' ) ) {
			if ( isset( $view->checkin->notification ) && $view->checkin->notification <> '' ) {
				echo SQ_Classes_Helpers_Sanitize::cloudHtml( $view->checkin->notification );
				?>
				<script>
					(function ($) {
						//The cloud notification HTML is sanitized (all data-*/on* stripped) so its
						//dismiss button carries no behaviour. The dismiss is bound here, in trusted
						//plugin code with fixed parameters, so a tampered notice can never change
						//what it does.
						$(function () {
							$('#sq_notification').on('click', '.sq_save_ajax button, .sq_notification_dismiss', function (e) {
								e.preventDefault();
								$('#sq_notification').hide();
								$.post(ajaxurl, {
									action: 'sq_ajax_seosettings_save',
									input: 'sq_notification',
									value: 0,
